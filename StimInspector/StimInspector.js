...
export class StimInspector {
	constructor (pjsWin, pjsLib) {
		...
		this._dom = undefined;
		this._grabbed = false;
		this._inspectorOverlay = new InspectorOverlay(pjsWin, pjsLib);
		this._init();
	}
	...
	_getStyles () {
		...
		.stim-inspector .toolbox .btn.toggled {
			background: #58a1dd;
		}
		...
		`<div class="stim-inspector">
			...
			<div class="content-area">
				<div class="toolbox">
					...
					<div class="toggle-overlay btn" title="toggle overlay">SO</div>
				</div>
	...
	_onShowOverlayClick (e) {
		e.currentTarget.classList.toggle("toggled");
		this._inspectorOverlay.toggleActive();
	}
	...
	_addEventListeners () {
		...
		const showOverlayBtn = dom.querySelector(".toggle-overlay.btn");
		...
		showOverlayBtn.addEventListener("click", this._onShowOverlayClick.bind(this));

		dom.addEventListener('wheel', this._onInspectorMouseWheel.bind(this));
		dom.addEventListener('click', this._onInspectorClick.bind(this));
